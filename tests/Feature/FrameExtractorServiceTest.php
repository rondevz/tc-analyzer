...
afterEach(function () use ($tmpBase): void {
    if (is_dir($tmpBase)) {
...
it('returns the path to the extracted frame on success', function () use ($tmpBase): void {
...
it('throws when ffprobe exits with non-zero', function () use ($tmpBase): void {
    $service = new class($tmpBase) extends FrameExtractorService {
        protected function exec(array $command): array { return [1, '', 'No such file']; }
    };

    expect(fn(): string => $service->extract('/tmp/missing.mp4', '@creator', 'vid123'))
        ->toThrow(RuntimeException::class, 'ffprobe failed');
});

it('throws when ffmpeg exits with non-zero', function () use ($tmpBase): void {
    $service = new class($tmpBase) extends FrameExtractorService {
        private int $call = 0;
    ...
    expect(fn(): string => $service->extract('/tmp/video.mp4', '@creator', 'vid123'))
        ->toThrow(RuntimeException::class, 'ffmpeg failed');
});

it('calls ffmpeg with seek time at 30% of video duration', function () use ($tmpBase): void {
    $service = new class($tmpBase) extends FrameExtractorService {
    ...
    $ssIndex = array_search('-ss', $service->ffmpegCommand, true);
    expect($service->ffmpegCommand[$ssIndex + 1])->toBe('6');
});

it('creates the output directory automatically', function () use ($tmpBase): void {
    $service = new class($tmpBase) extends FrameExtractorService {
        private int $call = 0;
