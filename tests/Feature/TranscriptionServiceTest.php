...
it('returns the transcript string on success', function () use ($base) {
...
it('returns empty string when whisper produces no output', function () use ($base) {
    mkdir($base, 0755, true);
    $videoPath = "{$base}/video.mp4";
    touch("{$base}/video.wav");
...
it('always deletes the wav file even when whisper fails', function () use ($base) {
    mkdir($base, 0755, true);
    ...
    expect(fn() => $service->transcribe($videoPath))
        ->toThrow(RuntimeException::class);

    expect(file_exists($wavPath))->toBeFalse();
});

it('throws a descriptive exception on ffmpeg failure', function () use ($base) {
    mkdir($base, 0755, true);
    $service = makeTranscriptionService([[1, 'No such file']]);

    expect(fn() => $service->transcribe("{$base}/video.mp4"))
        ->toThrow(RuntimeException::class, 'ffmpeg failed [1]: No such file');
});

it('throws a descriptive exception on whisper failure', function () use ($base) {
    mkdir($base, 0755, true);
    touch("{$base}/video.wav");
