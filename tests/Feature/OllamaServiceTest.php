...
it('generate returns the model response string on success', function () {
    Http::fake([
...
it('generate throws a descriptive exception on HTTP error', function () {
    Http::fake([
        '*/api/generate' => Http::response('Service Unavailable', 503),
    ]);

    expect(fn() => (new OllamaService())->generate('llama3.2:1b', 'Say hello'))
        ->toThrow(RuntimeException::class, '[503]');
});

it('generate throws when response key is absent from the JSON body', function () {
    Http::fake([
...
it('generateWithImage returns the model response string on success', function () {
    Http::fake([
...
it('generateWithImage sends the image as base64 in the request body', function () {
    Http::fake([
